...
from ..utils.utils_3d import split, merge, psi
...
class STConvLSTMCell(nn.Module):
    ...
    def forward(self, input, prev_state):
        ...
        if prev_state is None:
            ...
            prev_state = (
                torch.zeros(state_size, device=self.in_gate.weight.device, requires_grad=True),
                torch.zeros(state_size, device=self.in_gate.weight.device, requires_grad=True)
            )
...
class zig_rev_predictor(nn.Module):
    ...
    def init_hidden(self):
        ...
        for i in range(self.n_layers):
            hidden.append((torch.zeros(self.batch_size, self.hidden_size,self.temp,self.w,self.h, device=self._get_device(), requires_grad=True),
                           torch.zeros(self.batch_size, self.hidden_size,self.temp,self.w,self.h, device=self._get_device(), requires_grad=True)))
        return hidden
...
class CrevNet(nn.Module):
    def __init__(self, input_dim, output_dim, hidden_dims, kernel_size, img_width, img_height, num_layers, batch_size, mask_channel=1):
    ...
    self.frame_predictor = zig_rev_predictor(hidden_dims, hidden_dims, hidden_dims, num_layers, batch_size)
    self.encoder = autoencoder(nBlocks=[4,5,3], nStrides=[1, 2, 2], nChannels=None, init_ds=2, dropout_rate=0,
                              affineBN=True, in_shape=[input_dim, img_width, img_height])
    ...
    def forward(self, input_tensor, non_pred_feat=None, prediction_count=1):
        ...
        memo = torch.zeros(opt.batch_size, opt.rnn_size ,3, int(opt.image_width/8), int(opt.image_width/8), device=self._get_device(), requires_grad=True)
        ...
        for t in range(T):
            h = encoder(input_tensor[t - 1], True)
